...
- Isolated functional tests: system property `easy.disableAllPlugins=true` (`ExtensionRegistrar` at `easy-plugin-core/src/main/kotlin/com/mreil/easy/ExtensionRegistrar.kt:131` early-sets every `CanBeEnabled.enabled` to `false` after contributed extensions are attached). Tests must explicitly re-enable required extensions late via `easy { <name> { enabled.set(true) } }`, which overrides the early disable. For functional tests set it in both places: JUnit Pioneer `@SetSystemProperty(key = "easy.disableAllPlugins", value = "true")` on the test method and `systemProperty("easy.disableAllPlugins", "true")` in the `configure { }` block (forwarded as `-D` to the TestKit child via `runnerArguments`) (see `easy-plugin/src/functionalTest/kotlin/com/mreil/easy/DisableAllPluginsFuncTest.kt:14`). Not set globally — opt-in per test.
