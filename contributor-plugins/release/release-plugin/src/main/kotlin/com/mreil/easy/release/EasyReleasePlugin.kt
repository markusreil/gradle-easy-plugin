...
abstract class EasyReleasePlugin : AbstractEasyProjectPlugin() {
    ...
    private fun registerLifecycleService(
        target: Project,
        semverRelease: Provider<String>,
    ) {
        target.gradle.sharedServices.registerIfAbsent(EasyRelease.SERVICE_NAME, ReleaseLifecycleService::class.java) {
            it.parameters.releaseVersion.set(propertyResolver.get(RELEASE_VERSION_PROPERTY).orElse(semverRelease))
        }
    }
