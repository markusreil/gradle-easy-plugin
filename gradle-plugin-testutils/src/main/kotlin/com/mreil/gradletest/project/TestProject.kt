...
import com.mreil.gradletest.project.template.FileTemplate
import com.mreil.gradletest.project.template.GradlePropertiesTemplate
import com.mreil.gradletest.project.template.RawStringTemplate
import java.io.File
...
abstract class TestProject {
    ...
    /**
     * Configuration cache flag, staged into `gradle.properties`.
     * `null` omits the key. Defaults to `true` mirroring production `gradle.properties`.
     */
    var configurationCache: Boolean?
        get() = propertiesTemplate.configurationCache
        set(value) {
            propertiesTemplate.configurationCache = value
        }

    /** Shortcut to enable/disable configuration cache, e.g. `configurationCache = false`. */
    fun withConfigurationCache(enabled: Boolean): TestProject = apply { configurationCache = enabled }
